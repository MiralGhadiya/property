...
from app.deps import get_current_user, get_db, pagination_params
...
def create_feedback(
    ...
    try:
        ...
        db.rollback()
        logger.exception(f"Feedback creation failed user_id={current_user.id}")
        raise HTTPException(500, "Failed to submit feedback")


@router.get("/my", response_model=PaginatedResponse[FeedbackResponse])
def my_feedback(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
def update_my_feedback(
    ...
    if feedback.status != "OPEN":
        raise HTTPException(400, "Feedback can no longer be edited")
